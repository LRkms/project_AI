...
for category_name, category_code in category_mapping.items():
    ...
    # 네이버 쇼핑 라이브 페이지
    url = f'https://shoppinglive.kakao.com/categories?categoryId={category_code}'
    driver.get(url)
    ...
    # 스크롤 실행
    try:
        scroll_to_bottom(driver, max_scrolls=30)
    ...
    for i in range(1, 1501):
        xpath = f"/html/body/sl-app-root/sl-view-wrapper/div/main/article/sl-pw-categories/div/sl-view-player-list/div/cu-infinite-scroll/div/ul/sl-view-player-item[{i}]/li/div/a/div[2]/span"
        try:
            element = driver.find_element(By.XPATH, xpath)
            title = element.text.strip()  # 텍스트 추출 및 공백 제거
            if title:
                titles.append((title, category_name))
                if len(titles) % 50 == 0:
                    print(f"현재 {len(titles)}개 제목 수집됨")
        except Exception as e:
            print(f"item[{i}]에서 에러 발생: {e}")
            continue
    ...
    file_path = f'./crawling_data_01/shopping_category_{category_name}.csv'
